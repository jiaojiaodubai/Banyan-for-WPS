...
开发服务器运行期间，Ribbon 会显示“运行调试代码”按钮，它指向 `test/debug.ts`：一个快速、
随时改写、随时删除的人工临时调试入口。它是唯一需要弹窗反馈的入口（结果直接弹在眼前；
该弹窗是宿主模态，会阻塞页面直到点掉，因此只用于人工点击，不要接自动化），
默认展示运行环境与当前文档的域统计。

耗时的性能测试在 `test/index.ts`，由调试桥执行：

```shell
npm run test:wps
```

它在终端回显完整报告，同时把报告覆盖写入桌面的
`Banyan-for-WPS-performance.log`
（`[PASS]`/`[FAIL]`/`[TIME]`/`[SUMMARY]` 行可直接解析，终端按 `failed=` 设置退出码）。
测试会在 WPS 里创建临时文档，跑完 `test/` 下的 HTTP、域、citation、刷新和参考文献表测试
后无保存关闭。citation 测试通过 `test/automation.ts` 的无对话框注入工具构造 item/source
并调用字段工厂，不触发 `\style` 或 `\citation` GUI 路由。测试入口只在开发模式下加载。
测试入口自带闭环，不会留下残留文档：

- **互斥**：同一时刻只跑一轮，重复调用直接拒绝（桥不能强杀页面 JS，被中止的命令还会继续跑）；
- **清理**：临时文档带 `BANYAN_TEST_FIXTURE` 标记，开跑前先扫掉上次异常中断（刷新页面、
  重启宿主）留下的残留，只动带标记的文档；
- **宿主生命周期**：`test:wps` 带 `--launch` —— 没有页面接入时由桥启动 WPS，跑完由桥
  把它关掉；本来就开着的实例桥不会动（要保留调试用 `--keep-wps`）。

因此调试时不要在一轮测试跑着的时候刷新页面或重启 WPS：那会打断 `finally`，
残留要等下一轮开跑时才被清掉。
